...
function getErrorMessage(expect, error) {
    if (error) {
        if (error.getErrorMessage) {
            return error.getErrorMessage({ format: 'html' });
        } else if (error.output) {
            return error.output;
        } else {
            return expect.output.clone('html').error(error && error.message || '');
        }
    } else {
        return '';
    }
    ...
    return content.replace(codeBlockRegexp, function ($0, lang, code) {
        ...
        if (codeBlock.lang === 'output') {
            ...
            if (exampleBlock && exampleBlock.lang === 'javascript') {
                if (exampleBlock.errorMessage) {
                    output = exampleBlock.errorMessage.toString();
                }
            }
            return '```' + lang + '\n' + output + '\n```';
        } else {
                ...
                try {
                    ...
                    if (codeBlock.flags.async) {
                        ...
                        promise.then(function () {
                            ...
                            codeBlock.errorMessage = getErrorMessage(expect, e);
                            cb();
                        });
                    } else {
    ...
    function createUpdatedMarkdown(expect, content, codeBlocks, options) {
        ...
        renderer.code = function(code, blockInfoString, escaped) {
            ...
            if (lang === 'output') {
                if (exampleBlock && exampleBlock.lang === 'javascript') {
                    if (exampleBlock.errorMessage) {
                        return exampleBlock.errorMessage.toString('html')
